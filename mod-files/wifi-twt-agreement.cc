...
WifiTwtAgreement::WifiTwtAgreement(uint8_t flowId,
      ...
      m_nextServicePeriodEndEvent()
{
    NS_LOG_FUNCTION(
        this << "flowId:" << static_cast<int>(flowId) << "; peerMacAddress:" << peerMacAddress
             << "; isRequestingNode:" << isRequestingNode
             << "; isImplicitAgreement:" << isImplicitAgreement << "; flowType:" << flowType
             << "; isTriggerBasedAgreement:" << isTriggerBasedAgreement
             << "; isIndividualAgreement:" << isIndividualAgreement << "; twtChannel:" << twtChannel
             << "; wakeInterval:" << wakeInterval << "; nominalWakeDuration:" << nominalWakeDuration
             << "; nextTwt:" << nextTwt);
}
...
std::ostream&
operator<<(std::ostream& os, const WifiTwtAgreement& twt)
{
