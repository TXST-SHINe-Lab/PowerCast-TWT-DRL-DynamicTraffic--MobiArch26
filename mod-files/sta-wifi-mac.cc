...
{
    // Empty placeholder function scheduled at next expected beacon generation time at AP
    // This is used to calculate TWT schedules
}
...
StaWifiMac::GetTimeTillNextBeacon() const
{
    ...
    // Check if beacon tracking event is running
    if (m_nextExpectedBeaconGenerationEvent.IsPending())
    {
        Time delayLeft = Simulator::GetDelayLeft(m_nextExpectedBeaconGenerationEvent);
        NS_LOG_DEBUG("Time left till next beacon generation at AP = " << delayLeft.As(Time::US));
        return delayLeft;
    }
    else
    {
        // Beacon event not running (STA in power save or hasn't received beacon yet)
        NS_LOG_WARN("Beacon tracking event not running. Using default beacon interval estimate.");

        // Use standard beacon interval (102.4ms)
        // This is a safe fallback when beacon tracking is not active
        Time estimate = MicroSeconds(102400);
        NS_LOG_DEBUG("Estimated time till next beacon = " << estimate.As(Time::MS));
        return estimate;
    }
}
...
{
    NS_LOG_FUNCTION(this);
    NS_LOG_DEBUG("Setting PHY sleep to: " << enable << "for STA:" << this->GetAddress()
                                          << ", at time:" << Simulator::Now().As(Time::S));

    // Case 1: If Tx timer is running, it means STA is sending or has sent a frame that requires a
    // response.
    bool isTxTimerRunning = GetFrameExchangeManager()->GetWifiTxTimer().IsRunning();
    if (isTxTimerRunning)
    {
        // TxTimer is running. Get delay until it ends. Reschedule the sleep/wake-up event after the
        // TxTimer ends
        Time delayLeft = GetFrameExchangeManager()->GetWifiTxTimer().GetDelayLeft();
        NS_LOG_DEBUG(
            "TxTimer is running. Rescheduling SetPhySleepState after TxTimer ends at time from now:"
            << delayLeft.As(Time::US));
